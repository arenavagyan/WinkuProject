<template>
  <div class="post flex justify-center flex-col w-full rounded-mg bg-white p-2 pb-8 rounded border ">
    ...
    <div class="description_box w-full">
      ...
      <div class="image_descriprion" v-if="props.photoUrl">
        <p class=" description text-[.8rem] p-2 ml-2">{{props.description}}</p>
      </div>

      <div class="only_description" v-else>
        <p class="big_description description text-[1.2rem] p-2 ml-[.4rem]">{{props.description}}</p>
      </div>
    </div>
    ...
    <div class="w-full flex justify-center items-center p-2">
      ...
      <form action="" method="post" class="w-full ml-1 mt-2 ">
        <input type="text"
          ...
          placeholder="Post your comment" @keydown.enter="addComment">
      </form>
    </div>
